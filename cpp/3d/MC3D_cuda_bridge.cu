...
namespace ValoMC {
  ...
  template<typename T>
  void monte_carlo (MC3D<T>& mc3d, unsigned states_size) {
  ...
  template void monte_carlo<float> (MC3D<float>& mc3d, unsigned states_size);
  template void monte_carlo<double> (MC3D<double>& mc3d, unsigned states_size);
