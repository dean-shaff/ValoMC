 function test_MC3D (n_photons, check)
  ...
  [res0_d_0, res1_d_0, res2_d_0, res3_d_0, res4_d_0] = MC3Dmex(...
    ...
    phase0, n_photons, disable_pbar, rnseed, false, false);
  t_end_double = toc(t_start);
  double_res(1) = t_end_double;

  fprintf('%s CPU single precision version\n', text);
  t_start = tic;
  [res0_s_0, res1_s_0, res2_s_0, res3_s_0, res4_s_0] = MC3Dmex(...
    ...
    single(phase0), n_photons, disable_pbar, rnseed, false, false);
  t_end_single = toc(t_start);
  single_res(1) = t_end_single;
  ...
  [res0_d_1, res1_d_1, res2_d_1, res3_d_1, res4_d_1] = MC3Dmex(...
    H, HN, BH, r, BCType, BCIntensity, BCLightDirectionType,...
    BCLightDirection, BCn, mua, mus, g, n, f,...
    phase0, n_photons, disable_pbar, rnseed, false, true);
  t_end_double = toc(t_start);
  double_res(2) = t_end_double;

  fprintf('%s CPU single precision version\n', text);
  t_start = tic;
  [res0_s_1, res1_s_1, res2_s_1, res3_s_1, res4_s_1] = MC3Dmex(...
    H, HN, BH, single(r), BCType, single(BCIntensity), BCLightDirectionType,...
    single(BCLightDirection), single(BCn), single(mua), single(mus), single(g), single(n), single(f),...
    single(phase0), n_photons, disable_pbar, rnseed, false, true);
  t_end_single = toc(t_start);
  single_res(2) = t_end_single;
  ...
  [element_fluence, boundary_exitance, boundary_fluence, simulation_time, seed_used] = MC3Dmex(...
    H, HN, BH, r, BCType, BCIntensity, BCLightDirectionType,...
    BCLightDirection, BCn, mua, mus, g, n, f,...
    phase0, n_photons, disable_pbar, rnseed, true, false);
  ...
  [element_fluence, boundary_exitance, boundary_fluence, simulation_time, seed_used] = MC3Dmex(...
    H, HN, BH, single(r), BCType, single(BCIntensity), BCLightDirectionType,...
    single(BCLightDirection), single(BCn), single(mua), single(mus), single(g), single(n), single(f),...
    single(phase0), n_photons, disable_pbar, rnseed, true, false);
  ...
  if check
    fprintf('Double element_fluence allclose=%d\n', allclose(res0_d_0, res0_d_1, 1e-5, 1e-8));
    fprintf('Double boundary_exitance allclose=%d\n', allclose(res1_d_0, res1_d_1, 1e-5, 1e-8));

    fprintf('Single element_fluence allclose=%d\n', allclose(res0_s_0, res0_s_1, 1e-5, 1e-8));
    fprintf('Single boundary_exitance allclose=%d\n', allclose(res1_s_0, res1_s_1, 1e-5, 1e-8));
  end
